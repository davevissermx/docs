...
{{% alert color="info" %}}
Interrupting boundary events are as of 10.20 in an opt-in public beta. Please note that certain features or attributes may be subject to change and may contain bugs.{{% /alert %}}
...
{{% alert color="info" %}}In the current {{% /alert %}}

{{< figure src="/attachments/refguide/modeling/application-logic/workflows/workflow-how-to-boundary-events/radio-button.png" alt="Changing the type of boundary event" width="450" class="no-border">}}
...
The current release (10.20) of boundary events has the following functional limitations (which are actively being developed):
